...
def query():
    IDs = []                                                                                        # Empty List
    c = db.execute("select * from links where submitter_id = 62443 order by submitted_time asc ")   # where user = 62443 store it by time [asc --> ascending: From Low To High][des --> descending: From High To Low]
    for link_tuple in c:
        link = Link(*link_tuple)
        IDs.append(link.id)                 # Add id In This Empty List
    return IDs                              # Return This List
print query()
