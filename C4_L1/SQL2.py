from collections import namedtuple      # Lib For Make Class Have List
import sqlite3                         # Lib For database

# make a basic Link class
Link = namedtuple('Link', ['id', 'submitter_id', 'submitted_time', 'votes',     # submitter_id = User
                           'title', 'url'])

# list of Links to work with
links = [
    Link(0, 60398, 1334014208.0, 109,                                               # Link Is Constructor Of [Class Link]
...
for l in links:
    db.execute('insert into links values (?, ?, ?, ?, ?, ?)', l)          # insert l From Links List in This New Table
...
def query():
    c = db.execute("select * from links where submitter_id = 62443 and votes > 1000; ")     # [* = All Colomns from Table links where user = 62443 and votes > 1000]
#   link = Link(*c.fetchone())                              # * To Pass into Link Constructor and fetchone: To get first result Only have user = 62443 and  votes > 1000 And Don't Need To This For Loop
    for link_tuple in c:
        link = Link(*link_tuple)                            # Get All Value have user = 62443 and  votes > 1000
    return link.id                                          # Return id
print query()
