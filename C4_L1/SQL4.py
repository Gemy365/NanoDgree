...
def build_link_index():
    ...
    for l in links:
        index[l.id] = l                             index[l.id] --> Key = l --> value
    return index

def link_by_id(link_id):                            # Method Have Parameter ID
    for link in links:
        if link_id == link.id:                      # If This Parameter == id in this List
            return link                             # Return All Columns That Have This ID
